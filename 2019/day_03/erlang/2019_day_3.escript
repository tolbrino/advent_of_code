...
main(_) ->
  ...
  Result2 = result_1(solve_2(Input1)),
...
prepare_1(Inputs) ->
  Count = length(Inputs),
  lists:zip(lists:seq(1, Count), [string:tokens(I, ",") || I <- Inputs]).

solve_1(Wires) ->
  solve_1(Wires, #{}).

solve_1([], Plan) ->
  lists:sort([abs(PosX) + abs(PosY) || {{PosX, PosY}, {_, N, _}} <- maps:to_list(Plan), N > 1]);

solve_1([{Id, Wire} | Rest], Plan) ->
  Plan1 = run_wire(Id, Wire, Plan),
  solve_1(Rest, Plan1).
...
solve_2([], Plan) ->
  lists:sort([lists:sum(maps:values(Steps)) || {_, {_, N, Steps}} <- maps:to_list(Plan), N > 1]);

solve_2([{Id, Wire} | Rest], Plan) ->
  Plan1 = run_wire(Id, Wire, Plan),
  solve_2(Rest, Plan1).

run_wire(Id, Wire, Plan) ->
  {_, Plan1, _} = lists:foldl(
                 fun([D|N], {AccPos, AccPlan, AccSteps}) ->
                     {N1, _} = string:to_integer(N),
                     move(N1, D, AccPos, AccPlan, Id, AccSteps)
                 end, {{0, 0}, Plan, 0}, Wire),
  Plan1.

move(0, _D, Pos, Plan, _Line, Steps) ->
  {Pos, Plan, Steps};
move(N, $R=D, {PosX, PosY}, Plan, Line, Steps) ->
  Pos = {PosX+1, PosY},
  move(N-1, D, Pos, update_plan(Line, Pos, Plan, Steps+1), Line, Steps+1);
move(N, $L=D, {PosX, PosY}, Plan, Line, Steps) ->
  Pos = {PosX-1, PosY},
  move(N-1, D, Pos, update_plan(Line, Pos, Plan, Steps+1), Line, Steps+1);
move(N, $U=D, {PosX, PosY}, Plan, Line, Steps) ->
  Pos = {PosX, PosY+1},
  move(N-1, D, Pos, update_plan(Line, Pos, Plan, Steps+1), Line, Steps+1);
move(N, $D=D, {PosX, PosY}, Plan, Line, Steps) ->
  Pos = {PosX, PosY-1},
  move(N-1, D, Pos, update_plan(Line, Pos, Plan, Steps+1), Line, Steps+1).

update_plan(Line, K, Plan, Steps) ->
  {Visited, _, StepsRecord} = maps:get(K, Plan, {sets:new(), 0, #{}}),
  Visited1 = sets:add_element(Line, Visited),
  StepsRecord1 = case maps:is_key(Line, StepsRecord) of
                   true ->
                     StepsRecord;
                   false ->
                     maps:put(Line, Steps, StepsRecord)
                 end,
  maps:put(K, {Visited1, sets:size(Visited1), StepsRecord1}, Plan).

test() ->
  159 = result_1(solve_1(prepare_1(["R75,D30,R83,U83,L12,D49,R71,U7,L72",
              "U62,R66,U55,R34,D71,R55,D58,R83"]))),
  135 = result_1(solve_1(prepare_1(["R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51",
              "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"]))),
  610 = result_1(solve_2(prepare_1(["R75,D30,R83,U83,L12,D49,R71,U7,L72",
              "U62,R66,U55,R34,D71,R55,D58,R83"]))),
  410 = result_1(solve_2(prepare_1(["R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51",
              "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"]))),
  ok.
